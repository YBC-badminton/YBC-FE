            ...
            {!selectedActivity ? (
                <div className="space-y-4 sm:space-y-6">
                    <header>
                        ...
                        <p className="text-sm sm:text-base text-gray-500 font-medium">선택한 활동의 대진표를 작성하세요.</p>
                    </header>
                    <div className="grid grid-cols-1 gap-4">
                        {mockActivities.map((activity) => (
                            <div key={activity.id} onClick={() => handleSelectActivity(activity as any)} className="bg-white p-6 rounded-[24px] border border-gray-100 shadow-sm hover:border-blue-500 cursor-pointer transition-all relative">
                                ...
                                <p className="text-sm text-gray-400 mt-1">날짜: {activity.date} | 인원: {activity.participants.length}명</p>
                            </div>
                        ))}
                    </div>
                </div>
                    ...
                    <section className="bg-white rounded-2xl shadow-sm p-4 sm:p-8">
                        <header className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4 mb-6 sm:mb-8">
                            <div>
                                ...
                                <button onClick={() => setSelectedActivity(null)} className="text-gray-400 text-sm font-bold mt-2 hover:text-gray-600">← 목록으로</button>
                            </div>
                        ...
                        <div className="flex bg-gray-100 rounded-lg p-1 w-full sm:w-fit mb-6 sm:mb-8 overflow-x-auto">
                            {['1코트', '2코트', '3코트', '4코트', '전체'].map(c => (
                                <button key={c} onClick={() => { setCurrentCourt(c); setMobileStep(1); }} className={`flex-1 sm:flex-none px-4 sm:px-8 py-2 rounded-md text-xs sm:text-sm font-bold whitespace-nowrap ${currentCourt === c ? 'bg-white shadow-md text-blue-600' : 'text-gray-400'}`}>
                                    {c}
                                </button>
                            ))}
                        </div>

                        {/* [데스크탑 UI 유지 + 모바일 Step 1] 인원 선택 섹션 */}
                            ...
                            <div className="animate-in fade-in">
                                <div className="mb-6 sm:mb-8">
                                    ...
                                    <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 gap-1.5 sm:gap-2">
                                        {unassigned.map((p) => (
                                            <button key={p.name} onClick={() => handleAssign(p)} className="bg-white border border-gray-200 px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg text-xs flex justify-between items-center hover:border-blue-500 shadow-sm transition">
                                ...
                                {currentCourt !== '전체' && (
                                    <div className="bg-blue-50/50 rounded-xl p-4 sm:p-6 border-2 border-dashed border-blue-100">
                                        <p className="text-xs font-bold text-blue-500 mb-3 sm:mb-4">현재 코트 배치 멤버 (드래그/탭하여 배치)</p>
                                        <div className="flex flex-wrap gap-1.5 sm:gap-2">
                                            {assignments[currentCourt].map(p => (
                                                <div 
                                                    ...
                                                    className="bg-white border-2 border-blue-100 px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl text-xs font-bold flex items-center gap-1.5 sm:gap-2 cursor-grab shadow-sm transition active:scale-95"
                                                >
                                                    <span>{p.name}</span>
                                ...
                                {isMobile && currentCourt !== '전체' && assignments[currentCourt].length >= 4 && (
                                    <button onClick={() => setMobileStep(2)} className="w-full mt-6 py-4 bg-blue-600 text-white rounded-xl font-bold shadow-lg">대진표 작성하러 가기 →</button>
                                )}
                            </div>
                        )}
                    </section>

                    {/* [데스크탑 UI 유지 + 모바일 Step 2] 대진표 섹션 */}
                    {currentCourt === '전체' ? (
                        /* --- 전체 코트 현황 뷰 --- */
                        <section className="animate-in fade-in duration-500 space-y-4 sm:space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 p-3 sm:p-4 bg-gray-50 rounded-3xl">
                                {Object.entries(courtBrackets).map(([courtName, rows]) => (
                                    <div key={courtName} className="bg-white border border-gray-100 rounded-[24px] p-4 sm:p-6 shadow-sm">
                                        <span className="text-base sm:text-lg font-bold text-gray-800 border-b pb-3 block mb-4">{courtName}</span>
                                        <div className="space-y-2 sm:space-y-3">
                                            {rows.map((row, idx) => (
                                                <div key={idx} className="flex items-center justify-between p-2 rounded-lg bg-gray-50 text-[11px] sm:text-xs font-bold text-gray-700">
                                                    <span>{row[0]?.name || '미정'} : {row[1]?.name || '미정'}</span>
                                                    <span className="text-[10px] text-gray-200">VS</span>
                                                    <span>{row[2]?.name || '미정'} : {row[3]?.name || '미정'}</span>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                        ...
                        (!isMobile || mobileStep === 2) && (
                            <section className="bg-white rounded-2xl shadow-sm p-4 sm:p-8 animate-in slide-in-from-right-4">
                                {/* 모바일 전용: Step 2에서 배정된 멤버를 다시 보여줌 */}
                                {isMobile && (
                                    <div className="mb-6 bg-blue-50/50 p-4 rounded-xl border-2 border-dashed border-blue-100">
                                        <p className="text-[10px] font-bold text-blue-500 mb-3 uppercase tracking-wider">배정 멤버 (탭하여 드랍 영역에 배치)</p>
                                        <div className="flex flex-wrap gap-1.5">
                                            {assignments[currentCourt].map(p => (
                                                <div key={p.name} className="bg-white border border-blue-100 px-2.5 py-1.5 rounded-lg text-[11px] font-bold shadow-sm">
                                                    {p.name}
                                                </div>
                                            ))}
                                        </div>
                                ...
                                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 sm:gap-6 mb-6">
                                    <div className="flex items-center gap-4">
                                        {isMobile && <button onClick={() => setMobileStep(1)} className="p-2 text-gray-400 font-bold">←</button>}
                                        <h3 className="text-base sm:text-xl font-bold text-gray-800">{currentCourt} 대진표</h3>
                                        <div className="flex items-center gap-2 bg-gray-50 px-3 py-1.5 rounded-xl border border-gray-100">
                                            <button onClick={() => handleMatchCount('minus')} className="w-7 h-7 flex items-center justify-center bg-white border rounded-lg">-</button>
                                            <span className="text-sm font-bold w-6 text-center">{courtBrackets[currentCourt].length}</span>
                                            <button onClick={() => handleMatchCount('plus')} className="w-7 h-7 flex items-center justify-center bg-white border rounded-lg">+</button>
                                        </div>
                                    </div>
                                    <div className="flex gap-2">
                                        <button onClick={handleRandomAssign} className="bg-blue-600 text-white px-4 py-2 rounded-xl text-xs sm:text-sm font-bold flex-1 sm:flex-none">랜덤 배치</button>
                                        <button onClick={() => {
                                            setCourtBrackets(prev => ({ ...prev, [currentCourt]: initialBracket() }));
                                        }} className="bg-white border text-red-600 px-4 py-2 rounded-xl text-xs sm:text-sm font-bold flex-1 sm:flex-none">비우기</button>
                                    </div>
                                </div>

                                <div className="bg-white rounded-[24px] p-3 sm:p-12 space-y-4 sm:space-y-6 shadow-md border border-gray-50">
                                    {courtBrackets[currentCourt].map((rowArr, rowIndex) => (
                                        <div key={rowIndex} className="flex items-center justify-center gap-1.5 sm:gap-4">
                                            ...
                                            <div className="flex items-center gap-1 sm:gap-4 flex-1">
                                                ...
                                                <span className="text-xl sm:text-3xl font-bold text-gray-200 mx-0.5 sm:mx-2">:</span>
                                                <div className="flex items-center gap-1 sm:gap-4 flex-1">
                                ...
                                {isMobile && (
                                    <div className="mt-8 flex gap-3">
                                        ...
                                        <button onClick={() => setCurrentCourt('전체')} className="flex-1 py-4 bg-blue-600 text-white rounded-xl font-bold">전체 확정</button>
                                    </div>
                                )}
                            </section>
                        )
        ...
        >
            {person ? (
                <>
                    ...
                    <button onClick={onClear} className="absolute -top-1.5 -right-1.5 bg-white border rounded-full w-4 h-4 flex items-center justify-center text-[10px] shadow-sm">✕</button>
                </>
